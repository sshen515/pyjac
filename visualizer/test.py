    ...
    for line in lines:
        if "-----" in line or not line:
            ...
            if 'Sessional' not in gpa:
                gpa["Sessional"] = [float(gpa_list[1])]
            else:
                gpa["Sessional"].append(float(gpa_list[1]))
            if 'Annual' in gpa_list:
                if 'Annual' not in gpa:
                    gpa["Annual"] = [float(gpa_list[3])]
                else:
                    gpa["Annual"].append(float(gpa_list[3]))
                if 'Cumulative' not in gpa:
                    gpa["Cumulative"] = [float(gpa_list[5])]
                else:
                    gpa["Cumulative"].append(float(gpa_list[5]))
            else:
                if 'Cumulative' not in gpa:
                    gpa["Cumulative"] = [float(gpa_list[3])]
                else:
                    gpa["Cumulative"].append(float(gpa_list[3]))
        else:
